...
Multiple options are available to control how [meshWorkspaces](/docs/meshcloud.workspace.html) can sign up to meshStack in self-service. meshStack can be configured to suit your organization's unique demands for sign up.
...
meshStack assigns a default quota to newly registered [meshWorkspaces](/docs/meshcloud.workspace.html) (see section above). Platform Operators can configure this default quota via `meshfed.web.customer.defaultQuota`:
...
The default only applies to newly registered [meshWorkspaces](/docs/meshcloud.workspace.html). [meshPartners](/docs/administration.index.html) can change the individual quotas for managed meshWorkspaces at any time using the [administration area](/docs/administration.workspaces.html#workspace-quota-management).
